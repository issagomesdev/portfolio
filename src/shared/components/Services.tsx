        ...
        return (
            <Box display={'flex'} flexDirection={'column'} padding={theme.spacing(3)} gap={theme.spacing(3)} bgcolor={theme.palette.background.paper} borderRadius={'4px'} boxShadow={themeName === 'light' ? 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px' : 'none'} width={'45%'}>
                <Box display={'flex'} gap={theme.spacing(2)} alignItems={'center'} {...(mediumScreen ? { flexDirection: 'column' } : { flexDirection: 'row' })}>
                    ...
                    {smallScreen ? <Box> <Typography color={theme.palette.secondary.main} textTransform={'uppercase'} fontFamily={'Staatliches'} fontSize={smallScreen ? theme.spacing(4) : theme.spacing(6)} lineHeight={smallScreen ? theme.spacing(4) : theme.spacing(6)} textAlign={mediumScreen ? 'center' : 'start'} sx={{ wordBreak: 'break-all' }}>{title1 + title2}</Typography> </Box> : <Box> <Typography color={theme.palette.secondary.main} textTransform={'uppercase'} fontFamily={'Staatliches'} fontSize={smallScreen ? theme.spacing(4) : theme.spacing(6)} lineHeight={smallScreen ? theme.spacing(4) : theme.spacing(6)} textAlign={mediumScreen ? 'center' : 'start'} sx={{ wordBreak: 'break-word' }}>{title1}</Typography>
                        <Typography color={theme.palette.secondary.main} textTransform={'uppercase'} fontFamily={'Staatliches'} fontSize={smallScreen ? theme.spacing(4) : theme.spacing(6)} lineHeight={smallScreen ? theme.spacing(4) : theme.spacing(6)} textAlign={mediumScreen ? 'center' : 'start'} sx={{ wordBreak: 'break-word' }}>{title2}</Typography> </Box>}
