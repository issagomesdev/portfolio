...
const PortfolioComponent = () => {
    ...
    const ProjectComponent = (project: Project) => {
        return (
            <Box display={'flex'} width={smallScreen? '80%' : '60%'} gap={smallScreen || mediumScreen? theme.spacing(7) : theme.spacing(8)} {...(smallScreen || mediumScreen ? {flexDirection: 'column-reverse', alignItems: project.id % 2 === 0? 'flex-end' : 'flex-start'} : {flexDirection: project.id % 2 === 0? 'row' : 'row-reverse', alignItems: 'flex-start'})}{...(smallScreen? {right: '5%'} : {})}>
                <Box width={smallScreen? '100%' : '80%'} display={'flex'} flexDirection={'column'} gap={theme.spacing(1)}>
                    <Box display={'flex'} flexDirection={'column'} alignItems={(smallScreen || mediumScreen) && project.id % 2 === 0? 'flex-end' : 'flex-start'}>
                        <Typography variant="projectNumber" color={theme.palette.primary.main} {...(smallScreen? {fontSize: theme.spacing(4.5), lineHeight: theme.spacing(4.5)} : {})}>Projeto {project.id}</Typography>
                        <Typography variant="projectName" textAlign={(smallScreen || mediumScreen) && project.id % 2 === 0? 'end' : 'start'} {...(smallScreen? {fontSize: theme.spacing(3.5), lineHeight: theme.spacing(3.5)} : {})}>Storager - Your Store Manager</Typography>
                    </Box>
    ...
    return (
        <Box display={'flex'} flexDirection={'column'} justifyContent={'center'} gap={theme.spacing(smallScreen ? 8 : mediumScreen ? 10 : 15)} width={'100%'}>
            <Box display={'flex'} flexDirection={'column'} alignItems={'center'} gap={theme.spacing(2)}>
                ...
                <Box display={'flex'} width={'fit-content'} marginX={theme.spacing(1.5)} {...(smallScreen? {justifyContent: 'center', flexWrap: 'wrap'} : {justifyContent: 'space-between'})} columnGap={theme.spacing(4)}>
